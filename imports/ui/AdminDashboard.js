...
export const AdminDashboard = (props) => {
    return (
        <div className="" >
            <PrivateHeader title={renderTitle(props)} praxis={getPraxis(props)}/>
            <ChangePraxis/>
            <div className="dashboard-content">
                <div className="boxed-view__dashboard">
                                          
                    {checkSelected(props) ? <Link className='boxed-view__dashboardbox' to={{pathname: `/termine/${props.praxisId}`}}><h3>Mein Kalender</h3></Link> : undefined}
                    {checkSelected(props) ? <Link className='boxed-view__dashboardbox' to={{pathname: `/wartezimmer/${props.praxisId}`}}><h3>Mein Wartezimmer</h3></Link> : undefined}
...
export default withTracker( () => {
    ...
    const praxis = Praxen.findOne({_id: praxisId});
    if(praxis){
        return { 
            praxisId, 
            my_praxis: { 
